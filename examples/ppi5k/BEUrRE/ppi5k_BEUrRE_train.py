...
def main(test_only=False):
    ...
    """
    seed_everything()
    data = ppi5k.load_data('data', num_neg=30, batch_size=1024)
    rule_config = {
    ...
    regularization = {'delta': 0, 'min': 0, 'rel_trans': 0, 'rel_scale': 0,
                      'inverse': 0, 'transitive': 0, 'composite': 0}
    model = BEUrRE(data.num_ent, data.num_rel, emb_dim=200, gumbel_beta=0.01)
    ...
    trainer = BEUrRETrainer(data, model, loss, optimizer, early_stop,
                            save_path="./out_pt/BEUrRE/ppi5k/ppi5k-beurre-ndcg-rule1.pt")
    if not test_only:
