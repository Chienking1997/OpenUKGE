...
def main(test_only=False):
    ...
    """
    seed_everything()
    data = ppi5k.load_data('data', num_neg=10, batch_size=512)
    ...
    early_stop = EarlyStop(patience=2, monitor="wmr", mode="min", monitor_mode="tail")
    trainer = PASSLEAFTrainer(data, model, loss, optimizer, early_stop,
                              save_path="./out_pt/passleaf/ppi5k/ppi5k-passleaf-distmult-wmr.pt",
                              t_new_semi=20, t_semi_train=30)
    if not test_only:
        trainer.fit(epochs=2000, eval_freq=5)
        trainer.test()
    else:
        trainer.test()
