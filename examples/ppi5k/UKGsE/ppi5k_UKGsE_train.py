...
def main(test_only=False):
    seed_everything()
    data = ppi5k.load_data('data', num_neg=1, batch_size=64)
    if test_only:
        w2v_embedding, word2idx = None, None
    else:
        trainer = Word2VecTrainer(data.train, emb_dim=100, window=2,
            sg=0, neg_num=5, lr=0.001, batch_size=128, epochs=8)
        w2v_embedding, word2idx = trainer.train()
    model = UKGsE(w2v_embedding, word2idx, data.num_ent, data.num_rel, emb_dim=100)
    loss = UKGsELoss()
    ...
    trainer = UKGsETrainer(data, model, loss, optimizer, early_stop,
                           save_path="./out_pt/UKGsE/ppi5k/ppi5k-UKGsE-mse.pt")
    if not test_only:
