...
def main(test_only=False):
    ...
    """
    seed_everything(888)
    data = nl27k.load_data('data', num_neg=10, batch_size=1024)
    model = UKGEPSL(data.num_ent, data.num_rel, emb_dim=256, reg_scale=0.005)
    loss = UKGEPSLLoss()
    ...
    trainer = UKGEPSLTrainer(data, model, loss, optimizer, early_stop, psl=True,
                             save_path="./out_pt/UKGE_PSL/nl27k/nl27k-ukgepsl-ndcg.pt")
    if not test_only:
