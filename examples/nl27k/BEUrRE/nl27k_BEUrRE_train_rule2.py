...
def main(test_only=False):
    ...
    """
    seed_everything()
    data = nl27k.load_data('data', num_neg=30, batch_size=256)
    rule_config = {
            'transitive': {
                'use': True,
    ...
    model = BEUrRE(data.num_ent, data.num_rel, emb_dim=150, gumbel_beta=0.0001)
    loss = BEUrRELoss(rule_config=rule_config, regularization=regularization)
    opt = {'optimizer': {'type': 'Adam', 'lr': 1e-4}, }
    ...
    trainer = BEUrRETrainer(data, model, loss, optimizer, early_stop,
                            save_path="./out_pt/BEUrRE/nl27k/nl27k-beurre_ndcg-rule2.pt")
    if not test_only:
