...
def main():
    ...
    trainer = UPGATTrainer(data, model, loss, optimizer,early_stop,teacher_model=True,
                           save_path="./out_pt/UPGAT/nl27k/teacher/nl27k-upgat-mae-teacher.pt")
    trainer.fit(epochs=1000, eval_freq=1)
    trainer.test()
    trainer.pseudo_tail_predict(output_file_path='./data/nl27k/extra/pseudo.tsv')

def main2(test_only=False):
    seed_everything(888)
    data = nl27k.load_data('data', num_neg=10, batch_size=512,use_pseudo=True)
    model = UPGAT(data.num_ent, data.num_rel, emb_dim=512)
    loss = UPGATLoss()
    opt = {'optimizer': {'type': 'Adam', 'lr': 0.001, 'betas': (0.9, 0.99)},
    'scheduler': {'type': 'MultiStepLR', 'milestones': [12], 'gamma': 0.1}}
    optimizer = OptimBuilder(opt)
    early_stop = EarlyStop(patience=5, monitor="mae", mode="min", monitor_mode="tail")
    trainer = UPGATTrainer(data, model, loss, optimizer,early_stop, teacher_model=False,
                           save_path="./out_pt/upgat/nl27k/student/nl27k-upgat-mae-student.pt")
    if not test_only:
        trainer.fit(epochs=1000, eval_freq=1)
        trainer.test()
    else:
