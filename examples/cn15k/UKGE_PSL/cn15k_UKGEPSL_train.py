...
def main(test_only=False):
    ...
    """
    seed_everything(888)
    data = cn15k.load_data('data', num_neg=10, batch_size=1024, use_psl=True)
    ...
    early_stop = EarlyStop(patience=2, monitor="ndcg", mode="max", monitor_mode="tail")
    trainer = UKGEPSLTrainer(data, model, loss, optimizer, early_stop, psl=True,
                             save_path="./out_pt/UKGE_PSL/cn15k/cn15k-ukgepsl-ndcg.pt")
    if not test_only:
