...
def main(test_only=False):
    seed_everything()
    data = cn15k.load_data('data', num_neg=1, batch_size=128)
    if test_only:
        w2v_embedding, word2idx = None, None
    else:
        trainer = Word2VecTrainer(data.train, emb_dim=128, window=2,
            sg=1, neg_num=5, lr=0.001, batch_size=128, epochs=8)
    ...
    early_stop = EarlyStop(patience=3, monitor="wmrr", mode="max", monitor_mode="tail")
    trainer = UKGsETrainer(data, model, loss, optimizer, early_stop,
                           save_path="./out_pt/UKGsE/cn15k/cn15k-UKGsE-wmrr.pt")
    if not test_only:
