...
from ..evaluation import mean_ndcg
...
class UKGsETrainer:
    def __init__(self, data=None, model=None, loss=None, opt=None, early_stop=None, save_path=None, config=None):
        ...
        self.device, self.n_gpu = prepare_device()
        ...
        self.config = config
        self.optimizer = opt.create_optimizer(model=self.model)
        self.scheduler = opt.create_scheduler(optimizer=self.optimizer)
    ...
    def train_epoch(self):
        ...
        for train_batch in train_bar:
            ...
            # if self.n_gpu > 0:
            #     regularization = self.model.module.regularization(pos_sample)
            # else:
            # regularization = self.model.regularization(pos_sample)
            loss = self.loss(pos_score, neg_score, pro)
            loss.backward()
            self.optimizer.step()
