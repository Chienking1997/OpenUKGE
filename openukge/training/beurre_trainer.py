...
class BEUrRETrainer:
    ...
    def train_epoch(self):
        ...
        for train_batch in train_bar:
            ...
            loss = self.loss(pos_sample, neg_sample, pro, self.model, self.device)
            loss.backward()
            self.optimizer.step()
        ...
        with torch.no_grad():
            ...
            if monitor == 'mse' or monitor == 'mae':
                ...
                (mr_raw, mrr_raw, hit1_raw, hit3_raw, hit10_raw,
                 mr_filter, mrr_filter, hit1_filter, hit3_filter, hit10_filter) = result
                return mrr_raw
            elif monitor == 'wmrr' or monitor == 'wmr':
                ...
                linear_ndcg, exp_ndcg = mean_ndcg(self.valid["hr_map"], self.model, self.device)
                return linear_ndcg
